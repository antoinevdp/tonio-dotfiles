...
Item {
    ...
    function applyWallpaper(safeFileName, isVideo) {
        ...
        const logFile = paths.logDir + "/wallpaper_picker.log";
            ...
            if (alreadyExists) {
                const applyScript = `
                    ...
                    if ! pgrep -x awww-daemon >/dev/null; then
                        awww init >> ${logFile} 2>&1 || true
                    fi

                    if [ "$TARGET_MONITORS" = "all" ]; then
                        awww img "$DEST_FILE" --transition-type ${randomTransition} --transition-fps 60 --transition-duration 1 >> ${logFile} 2>&1 &
                    else
                        awww img -o "$TARGET_MONITORS" "$DEST_FILE" --transition-type ${randomTransition} --transition-fps 60 --transition-duration 1 >> ${logFile} 2>&1 &
                    fi
                    
                    ( matugen image "$FINAL_THUMB" --type scheme-tonal-spot --mode dark --source-color-index 0 || true; bash "$RELOAD_SCRIPT" || true ) &
                `;
        ...
        if (isVideo) {
            wallpaperCmd = `
                echo "" >> ${logFile}
                echo "[$(date +'%H:%M:%S.%3N')] SKIPPING LOCAL VIDEO: ${escOriginal}; mpvpaper is not installed" >> ${logFile}
            `;
        } else {
            wallpaperCmd = `
                echo "" >> ${logFile}
                ...
                if [ "${escOutputs}" = "all" ]; then
                    awww img "${escOriginal}" --transition-type ${randomTransition} --transition-fps 60 --transition-duration 1 >> ${logFile} 2>&1 &
                else
                    awww img -o "${escOutputs}" "${escOriginal}" --transition-type ${randomTransition} --transition-fps 60 --transition-duration 1 >> ${logFile} 2>&1 &
                fi
            `;
        }
    ...
    Shortcut { sequence: "Escape"; enabled: !window.isApplying; onActivated: Qt.quit() }
